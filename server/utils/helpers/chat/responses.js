...
function handleDefaultStreamResponseV2(response, stream, responseProps) {
  ...
  return new Promise(async (resolve) => {
    let fullText = "";
    let reasoningText = ""; // Track reasoning content for proper tag handling
    ...
    try {
      for await (const chunk of stream) {
        ...
        // Handle reasoning content from AI providers that support it
        if (reasoningToken) {
        ...
        if (
          ...
          // Ensure reasoning tag is closed if message ends mid-reasoning
