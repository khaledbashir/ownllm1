...
const Workspace = {
  ...
  whereWithUser: async function (
    ...
    if ([ROLES.admin, ROLES.manager].includes(user.role)) {
      if (user.organizationId) {
        return await this.whereWithOrg(clause, user.organizationId, limit, orderBy);
      }
  ...
  replicate: async function (workspaceId, creatorId = null, newName = null) {
    try {
      ...
      const workspaceData = {
        ...
        ancProductCatalog: sourceWorkspace.ancProductCatalog,
        activeLogicModule: sourceWorkspace.activeLogicModule,
