module.exports.runtime = {
    handler: async function ({ action, params }) {
        ...
        try {
            switch (action) {
                case "list_leads": {
                    this.introspect(`${callerId}: Listing CRM leads...`);
                ...
                case "create_lead": {
                    this.introspect(`${callerId}: Creating lead "${params.name}"...`);
                    const pipeline = await prisma.crm_pipelines.findFirst({ where: { userId } });
                    if (!pipeline) return JSON.stringify({ success: false, error: "No pipeline found" });
                ...
                case "update_lead": {
                    ...
                    this.introspect(`${callerId}: Updating lead #${id}...`);
                ...
                case "move_lead": {
                    ...
                    this.introspect(`${callerId}: Moving lead #${id} to ${stage}...`);
                    const card = await prisma.crm_cards.update({
                        where: { id: Number(id), userId },
                        data: { stage }
                    });
                ...
                case "delete_lead": {
                    const { id } = params;
                    this.introspect(`${callerId}: Deleting lead #${id}...`);
