  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        response
          .status(500)
          .json({ success: false, error: "Could not save artifact" });
      }
    }
  );
  ...
  app.get(
    ...
    async (_request, response) => {
      try {
        ...
        response
          .status(500)
          .json({ success: false, error: "Could not fetch artifacts" });
      }
    }
  );
    ...
    async (request, response) => {
      try {
        ...
        if (!id) {
          response
            .status(400)
            .json({ success: false, error: "Invalid artifact id" });
          return;
        }
        ...
        response
          .status(500)
          .json({ success: false, error: "Could not delete artifact" });
      }
    }
  );
}
