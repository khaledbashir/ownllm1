...
function blockTemplatesEndpoints(app) {
    ...
    app.get("/workspace/:slug/block-templates", validatedRequest, async (req, res) => {
        try {
            ...
            res
                .status(500)
                .json({ error: "Failed to list block templates", details: error.message });
        }
    });
    ...
    // Create template
    app.post("/workspace/:slug/block-templates", validatedRequest, async (req, res) => {
        try {
            const { slug } = req.params;
            ...
            if (!name || !snapshot) {
                return res.status(400).json({ error: "Name and snapshot are required" });
            }
            ...
            const template = await prisma.block_templates.create({
                data: {
                    name,
                    description,
                    snapshot: typeof snapshot === 'object' ? JSON.stringify(snapshot) : snapshot,
