...
import { Sparkle, Plus, Trash, CaretDown, CaretRight } from "@phosphor-icons/react";
...
export default function DocAISettings({ workspace, setHasChanges }) {
    ...
    const [newAction, setNewAction] = useState({ name: "", prompt: "", icon: "✨" });
    ...
    const handleActionChange = (actionId, field, value) => {
        setCustomActions(customActions.map(a =>
                ...
                <div className="mt-3 pl-6 space-y-4 border-l-2 border-white/10 ml-2">
                    ...
                    <div>
                            ...
                            {customActions.map((action) => (
                                ...
                                >
                                    <div className="flex-1 space-y-2">
                                        ...
                                        <textarea
                                            value={action.prompt}
                                            onChange={(e) => handleActionChange(action.id, "prompt", e.target.value)}
                        ...
                        {customActions.length === 0 && !showAddAction && (
                            <p className="text-xs text-white/40 mt-1">
                                No custom actions. Add actions to extend Doc AI with your own prompts.
                            </p>
                        )}
                    </div>
                    ...
                    <div className="text-xs text-white/40 bg-white/5 rounded-lg p-3">
                        ...
                        <ul className="list-disc list-inside space-y-0.5">
                            <li><code className="text-purple-300">{"{{selectedText}}"}</code> - Currently selected text</li>
                            <li><code className="text-purple-300">{"{{context}}"}</code> - Document content above cursor</li>
                            <li><code className="text-purple-300">{"{{prompt}}"}</code> - User's additional input</li>
                        </ul>
                    </div>
                </div>
            )}
