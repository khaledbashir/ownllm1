...
export default function AddServerModal({ onClose, onAdd }) {
  ...
  const handleSubmit = async () => {
    ...
    const { success, error } = await MCPServers.addServer({
    ...
    if (success) {
      showToast("MCP server added successfully!", "success", { clear: true });
      onAdd();
      onClose();
    } else {
      showToast(error || "Failed to add MCP server", "error", { clear: true });
    }
  };
            ...
            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm text-green-500">
                  Configuration is valid and ready to add
                </p>
              </div>
            </div>
          )}
        ...
        <div className="sticky bottom-0 bg-theme-bg-secondary border-t border-white/10 px-6 py-4 rounded-b-2xl z-10">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 px-6 py-3 rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 text-white font-medium hover:from-purple-700 hover:to-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg disabled:shadow-none"
            >
              {loading ? (
                <span className="flex items-center gap-2">
                  <div className="w-4 h-4 border-2 border-white/30 border-t-transparent rounded-full animate-spin" />
