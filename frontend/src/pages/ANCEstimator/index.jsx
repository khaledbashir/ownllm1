    ...
    <div className="w-screen h-screen overflow-hidden bg-zinc-900 flex">
      ...
      <div className="flex-1 w-full h-full overflow-y-scroll p-6 text-white">
        <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-blue-400">ANC Estimator module</h1>
            <div className="text-xs text-gray-500">Workspace: {workspace?.name}</div>
        </div>
        ...
        {activeTab === "catalog" && (
            <div className="max-w-6xl">
                <div className="mb-4 flex justify-between items-center">
                    <p className="text-gray-400 text-sm">Define your LED/Construction products and their margins here.</p>
                    <div className="space-x-2">
                        <button onClick={addProduct} className="bg-green-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700 transition">Add Product</button>
                        <button onClick={handleSaveCatalog} className="bg-blue-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition">Save Catalog</button>
                    </div>
                </div>
                        ...
                        <tbody className="divide-y divide-gray-700">
                            ...
                            {catalog.map((product, idx) => (
                                <tr key={idx} className="hover:bg-gray-750 transition">
                                    ...
                                    <td className="p-3"><input type="number" step="0.01" value={product.fixedCosts} onChange={(e) => handleCatalogChange(idx, 'fixedCosts', parseFloat(e.target.value))} className="bg-gray-700 border border-gray-600 px-2 py-1 rounded w-32 text-white focus:border-blue-500 outline-none" /></td>
                                    <td className="p-3">
                                        <button onClick={() => {
                                            const newCatalog = [...catalog];
                                            newCatalog.splice(idx, 1);
                                            setCatalog(newCatalog);
                                        }} className="text-red-400 hover:text-red-300 font-medium text-xs">Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
        ...
        {activeTab === "estimator" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl">
                <div className="bg-gray-800 border border-gray-700 p-6 rounded-lg shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 text-white border-b border-gray-700 pb-2">1. Input Parameters</h2>
                    <div className="space-y-5">
                        <div>
                            <label className="block text-sm font-medium text-gray-400 mb-1">Select Product</label>
                            <select 
                                value={inputs.selectedProductIndex}
                                onChange={(e) => setInputs({...inputs, selectedProductIndex: Number(e.target.value)})}
                            ...
                            >
                                <option value={-1} disabled>-- Select a Product from Catalog --</option>
                                {catalog.map((p, idx) => (
                                    <option key={idx} value={idx}>{p.productName}</option>
                                ))}
                            </select>
                            {catalog.length === 0 && <p className="text-xs text-yellow-500 mt-1">Catalog is empty. Go to "Product Catalog" tab to add items.</p>}
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-400 mb-1">Width (ft)</label>
                                <input type="number" step="0.1" value={inputs.width} onChange={(e) => setInputs({...inputs, width: e.target.value})} className="w-full bg-gray-900 border border-gray-600 p-2.5 rounded-lg text-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-400 mb-1">Height (ft)</label>
                                <input type="number" step="0.1" value={inputs.height} onChange={(e) => setInputs({...inputs, height: e.target.value})} className="w-full bg-gray-900 border border-gray-600 p-2.5 rounded-lg text-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-400 mb-1">Quantity</label>
                            <input type="number" value={inputs.quantity} onChange={(e) => setInputs({...inputs, quantity: e.target.value})} className="w-full bg-gray-900 border border-gray-600 p-2.5 rounded-lg text-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="1" />
                        </div>
                        <button onClick={calculateQuote} className="w-full bg-gradient-to-r from-blue-600 to-blue-500 p-3 rounded-lg font-bold hover:from-blue-500 hover:to-blue-400 transition shadow-lg mt-2">Generate Quote</button>
                    </div>
                </div>

                <div className="bg-gray-800 border border-gray-700 p-6 rounded-lg shadow-lg flex flex-col">
                    <h2 className="text-xl font-semibold mb-4 text-white border-b border-gray-700 pb-2">2. Quote Preview</h2>
                    {quote ? (
                        <div className="space-y-6 flex-1">
                            <div className="bg-zinc-900 p-5 rounded-lg border border-zinc-700">
                                ...
                                <div className="flex justify-between text-lg mb-2">
                                    ...
                                    <span className="font-mono">${quote.client.unitPrice.toFixed(2)}</span>
                                </div>
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center text-gray-500 opacity-50">
                            <svg className="w-16 h-16 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
