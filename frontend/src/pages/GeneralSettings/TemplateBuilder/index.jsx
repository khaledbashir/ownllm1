...
const THINK_REGEX_COMPLETE =
  /<(think|thinking|thought)\s*>([\s\S]*?)<\/(think|thinking|thought)\s*>/gi;
    ...
    try {
      ...
      if (data.templateHtml) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: "✨ Template updated! Check the preview on the right.",
          },
        ]);
      } else {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: `Error: ${e.message}. Make sure your LLM is configured in Settings.`,
        },
      ]);
    } finally {
      setLoading(false);
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      if (res && (res.success !== false || res.template)) {
        ...
        showToast(
          res?.error || "Save failed. Check console for details.",
          "error"
        );
      }
    } catch (e) {
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 && (
                <div className="text-center py-12 text-theme-text-secondary">
                  <Sparkle
                    size={48}
                    className="mx-auto mb-4 text-yellow-400/50"
                  />
                  ...
                  <div className="mt-6 space-y-2 text-sm">
                    ...
                    <button
                      onClick={() =>
                        setInput(
                          "Create a professional proposal template for a web development agency"
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        setInput(
                          "Design an invoice template with a modern, minimal style"
                        )
                      }
                      className="block w-full text-left px-4 py-2 bg-theme-bg-primary rounded-lg hover:bg-theme-bg-secondary transition-colors"
                    >
                    ...
                    <button
                      onClick={() =>
                        setInput(
                          "Create a quote template for a marketing consultant"
                        )
                      }
                      className="block w-full text-left px-4 py-2 bg-theme-bg-primary rounded-lg hover:bg-theme-bg-secondary transition-colors"
                    >
        ...
        <div className="prose prose-invert prose-sm max-w-none">
          <ReactMarkdown
            ...
            components={{
              table: ({ node, ...props }) => (
                <table
                  className="min-w-full border border-theme-sidebar-border"
                  {...props}
                />
              ),
              th: ({ node, ...props }) => (
                <th
                  className="border border-theme-sidebar-border px-2 py-1 bg-theme-bg-secondary"
                  {...props}
                />
              ),
              td: ({ node, ...props }) => (
              ...
              code: ({ node, inline, ...props }) =>
                inline ? (
                  <code
                    className="bg-theme-bg-secondary px-1 rounded"
                    {...props}
                  />
                ) : (
                  <code
                    className="block bg-theme-bg-secondary p-2 rounded overflow-x-auto"
                    {...props}
                  />
                ),
            }}
