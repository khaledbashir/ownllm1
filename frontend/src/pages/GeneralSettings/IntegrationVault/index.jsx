...
function AddEditModal({ isOpen, onClose, onSave, editingEntry }) {
    ...
    useEffect(() => {
        if (editingEntry) {
            ...
            const fields = Object.entries(editingEntry.config || {}).map(([key, value]) => ({ key, value }));
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-theme-bg-secondary border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-5">
                    ...
                    {!editingEntry && (
                        <div>
                            <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">Quick Add</label>
                    ...
                    {/* Service ID */}
                    <div>
                        <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">Service ID</label>
                        <input
                            type="text"
                            value={service}
                            onChange={(e) => setService(e.target.value.toLowerCase().replace(/\s/g, "_"))}
                    ...
                    <div>
                        ...
                        >
                            {Object.entries(CATEGORIES).map(([key, val]) => (
                                <option key={key} value={key} className="bg-theme-bg-secondary">{val.label}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-xs text-white/50 uppercase tracking-wider">Credentials</label>
                            <button
            ...
            >
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-medium text-white">Integration Vault</h1>
                            <p className="text-xs text-white/40">Secure credentials for your AI agents</p>
                        </div>
                    </div>
                    <button
                        onClick={openAdd}
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-64 text-white/30">
                            ...
                            <p className="text-xs mt-1">Add API keys for your agents to use</p>
                        </div>
                    ) : (
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    <span className="text-xs text-white/40 font-mono">{entry.service}</span>
                                                    <span className="text-xs text-white/30">• {fieldCount} field{fieldCount !== 1 ? "s" : ""}</span>
                                                </div>
                                            </div>
                                        </div>
