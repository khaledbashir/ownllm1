...
import { User, Building, DollarSign, Clock, Edit3, Trash2, ExternalLink, MessageSquare } from "lucide-react";
import { API_BASE } from "@/utils/constants";

export default function KanbanCard({ card, onEdit, onDelete, onDragStart, isDragging }) {
    ...
    return (
        <div
            ...
            className={`relative group overflow-hidden rounded-xl bg-theme-bg-secondary border border-theme-sidebar-border hover:border-theme-text-secondary/50 cursor-grab active:cursor-grabbing transition-all duration-200 ${isDragging ? "opacity-50 scale-95 shadow-lg" : "hover:-translate-y-1 hover:shadow-md"
                }`}
        >
            <div className="p-4 space-y-3">
                ...
                <div className="flex justify-between items-start gap-2">
                    ...
                    {card.priority && (
                        <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-md border ${priorityConfig} shrink-0`}>
                            {card.priority}
                        </span>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-theme-sidebar-border/50">
                    {card.embedSessionId && (
                        <div className="p-1 rounded-md bg-blue-500/10 text-blue-400" title="From embedded chat">
