...
export default function OrganizationDetails({ organization, closeModal, onEdit }) {
  ...
  useEffect(() => {
    async function fetchOrganizationData() {
      ...
      const [statsData, usersData, workspacesData, seatsData] = await Promise.all([
        ...
        <div className="flex overflow-hidden max-h-[calc(90vh-80px)]">
          <div className="w-64 border-r border-white/10 p-4 overflow-y-auto">
            <div className="space-y-4">
              ...
              <div>
                <h4 className="text-sm font-semibold text-theme-text-primary mb-2">Plan</h4>
                <div className={`px-3 py-1.5 rounded inline-block ${PLAN_COLORS[organization.plan]}`}>
                  {organization.plan}
                </div>
              </div>

              <div>
                <h4 className="text-sm font-semibold text-theme-text-primary mb-2">Status</h4>
                <div className={`px-3 py-1.5 rounded inline-block ${STATUS_COLORS[organization.status]}`}>
                  {organization.status}
                </div>
              </div>

              <div>
                <h4 className="text-sm font-semibold text-theme-text-primary mb-2">Seat Limit</h4>
                <p className="text-sm text-theme-text-secondary">
                  {organization.seatLimit ? `${organization.seatLimit} users` : "Unlimited"}
                </p>
              </div>

              {remainingSeats !== null && (
                <div>
                  <h4 className="text-sm font-semibold text-theme-text-primary mb-2">Available Seats</h4>
                  <p className={`text-2xl font-bold ${remainingSeats > 0 ? 'text-green-400' : 'text-yellow-400'}`}>
                    {remainingSeats}
                  </p>
              ...
              <div>
                <h4 className="text-sm font-semibold text-theme-text-primary mb-2">Created</h4>
                <p className="text-sm text-theme-text-secondary">
                  ...
                  {stats && (
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white/5 rounded-lg p-4">
                        <p className="text-xs text-theme-text-secondary uppercase mb-1">Total Users</p>
                        <p className="text-2xl font-bold text-theme-text-primary">{stats.userCount}</p>
                      </div>
                      <div className="bg-white/5 rounded-lg p-4">
                        <p className="text-xs text-theme-text-secondary uppercase mb-1">Total Workspaces</p>
                        <p className="text-2xl font-bold text-theme-text-primary">{stats.workspaceCount}</p>
                      </div>
                      <div className="bg-white/5 rounded-lg p-4">
                        <p className="text-xs text-theme-text-secondary uppercase mb-1">Total Documents</p>
                        <p className="text-2xl font-bold text-theme-text-primary">{stats.documentCount}</p>
                      </div>
                      <div className="bg-white/5 rounded-lg p-4">
                        <p className="text-xs text-theme-text-secondary uppercase mb-1">Pending Invites</p>
                        <p className="text-2xl font-bold text-theme-text-primary">{stats.pendingInviteCount}</p>
                      </div>
                    </div>
                  )}
