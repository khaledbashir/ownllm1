  ...
  useEffect(() => {
    async function loadHotelPipeline() {
      try {
        ...
        const hotelPipeline = pipelines.find(
          (p) => p.name === "Contract Processing"
        );
        if (hotelPipeline) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/40 mt-1">
                Manage and verify AI-extracted hotel contract rates.
              </p>
            </div>
          ...
          <div className="bg-gradient-to-br from-[#1a1a1a] to-[#141414] border border-white/10 rounded-2xl p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10 max-w-2xl">
              <h2 className="text-2xl font-bold mb-4">
                Launch Z.ai Extraction
              </h2>
              <p className="text-white/60 mb-8 leading-relaxed">
                Upload a hotel contract PDF or paste the text in any workspace
                chat. Use the{" "}
                <span className="text-emerald-400 font-mono">/hotel-rates</span>{" "}
                command or Smart Actions to trigger the GLM-4.7 extraction
                engine.
              </p>
...
function StatCard({ title, value, icon, description, alert = false }) {
  return (
    <div
      className={`bg-[#1a1a1a] border ${alert ? "border-amber-500/30" : "border-white/5"} p-6 rounded-2xl hover:border-white/20 transition-all group`}
    >
...
function WorkflowStep({ number, title, desc }) {
  return (
    <div className="bg-[#1a1a1a]/50 border border-white/5 p-6 rounded-2xl relative overflow-hidden group hover:bg-[#1a1a1a] transition-all">
      ...
      <div className="font-bold mb-2 group-hover:text-emerald-400 transition-colors uppercase tracking-widest text-[10px]">
        Step {number}
      </div>
