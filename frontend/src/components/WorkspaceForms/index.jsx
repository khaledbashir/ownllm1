      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm">
              Create and manage AI-powered forms for this workspace.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {apps.map((form) => (
            <div
              key={form.uuid}
              className="bg-theme-bg-secondary rounded-xl p-5 border border-white/5 hover:border-blue-500/30 transition-all group relative"
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Link
                    to={paths.forms.builder(slug, form.uuid)}
              ...
              <div className="flex items-center justify-between pt-3 border-t border-white/5">
                ...
                {form.isPublic && (
                  <Link
                    to={paths.forms.public(form.uuid)}
                    target="_blank"
                    className="flex items-center gap-1 text-[10px] text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full"
                  >
          ...
          {apps.length === 0 && (
            ...
            >
              ...
              <p className="text-slate-400 text-xs">
                Start from scratch or use AI to generate one.
              </p>
            </div>
          )}
        </div>
