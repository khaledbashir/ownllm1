...
export function EditorProvider({ children }) {
  ...
  const isReady = useCallback(
    () => !!editorRef.current && !!docRef.current,
    []
  );
   ...
   */
  const injectParagraph = useCallback(
    (text) => {
      const doc = docRef.current;
   ...
   */
  const injectHeading = useCallback(
    (level, text) => {
      const doc = docRef.current;
      const noteBlock = getNoteBlock();
      if (!doc || !noteBlock) {
   ...
   */
  const injectListItem = useCallback(
    (text, type = "bulleted") => {
      const doc = docRef.current;
      const noteBlock = getNoteBlock();
      if (!doc || !noteBlock) return null;
   ...
   * ])
   */
  const injectBlocks = useCallback(
    (blocks) => {
          ...
          case "list":
            if (Array.isArray(block.items)) {
              for (const item of block.items) {
                results.push(
                  injectListItem(item, block.listType || "bulleted")
                );
              }
            }
            break;
   ...
   */
  const injectMeetingNotes = useCallback(
    (data = {}) => {
      const {
        title = "Meeting Notes",
      ...
      const blocks = [
        ...
        {
          type: "paragraph",
          text: `👥 Attendees: ${attendees.join(", ") || "TBD"}`,
        },
        { type: "divider" },
   ...
   */
  const injectProposal = useCallback(
    (data = {}) => {
      const {
      ...
      const blocks = [
        ...
        {
          type: "paragraph",
          text: summary || "Brief description of the project...",
        },
        { type: "divider" },
   ...
   */
  const injectQuickQuote = useCallback(
    (data = {}) => {
      const {
        title = "Quick Quote",
        items = [],
      ...
      const blocks = [
        ...
        ...(discount > 0
          ? [
              {
                type: "paragraph",
                text: `Discount (${discount}%): -$${discountAmount.toLocaleString()}`,
              },
            ]
          : []),
        { type: "paragraph", text: `**TOTAL: $${total.toLocaleString()}**` },
