...
import { CurrencyDollar, Plus, Trash, PencilSimple, FloppyDisk, X, FileCsv, SpinnerGap } from "@phosphor-icons/react";
...
export default function RateCardManager({ workspace }) {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            const TARGETS = {
                name: ['role', 'role name', 'rolename', 'title', 'position', 'item', 'name', 'job'],
                rate: ['rate', 'price', 'cost', 'hourly', 'amount', '$/hr', 'usd', 'eur'],
            ...
            for (let i = 0; i < Math.min(10, rawRows.length); i++) {
                ...
                row.forEach(cell => {
                    ...
                    for (const key of Object.keys(TARGETS)) {
                        if (TARGETS[key].some(alias => strCell.includes(alias) || levenshtein(strCell, alias) <= 2)) {
                            score++;
                        }
                    }
                });
            ...
            for (let i = bestHeaderRowIndex + 1; i < rawRows.length; i++) {
                ...
                if (catVal) {
                    const match = ROLE_CATEGORIES.find(c =>
            ...
            if (newRoles.length > 0) {
                ...
                alert("Could not find any valid roles to import. Please check your column headers.");
            }
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        {importing ? <SpinnerGap className="w-4 h-4 animate-spin" /> : <FileCsv className="w-4 h-4" />}
                        Import Excel
                    </button>
                    <button
                        onClick={addEntry}
            ...
            <div className="space-y-4">
                {rateCard.length === 0 && !newEntry && (
                    <div className="text-center py-12 text-white/40">
                        ...
                        <p className="text-sm">Add your first role, or import an Excel sheet.</p>
                    </div>
                )}
