        ...
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between bg-slate-800/50 p-4 rounded-lg border border-white/5">
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-full ${isPublic ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
              >
                {isPublic ? <Globe size={20} /> : <Lock size={20} />}
              </div>
              <div>
                <h3 className="text-white font-medium">
                  {isPublic ? "Public Access Enabled" : "Private Form"}
                </h3>
                <p className="text-slate-400 text-sm">
                  {isPublic
                    ? "Anyone with the link can view and submit."
                    : "Only accessible in workspace."}
                </p>
              </div>
            </div>
