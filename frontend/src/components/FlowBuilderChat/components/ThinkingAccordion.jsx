                ...
                <div className="bg-theme-bg-primary rounded-lg p-3 border-l-2 border-purple-500/50 overflow-y-auto max-h-[450px]">
                    <div className="prose prose-sm prose-invert max-w-none text-theme-text-secondary">
                        <ReactMarkdown
                            ...
                            components={{
                                table: ({ node, ...props }) => (
                                    <table className="min-w-full text-xs border border-theme-sidebar-border" {...props} />
                                ),
                                th: ({ node, ...props }) => (
                                    <th className="border border-theme-sidebar-border px-2 py-1 bg-theme-bg-secondary text-left" {...props} />
                                ),
                                td: ({ node, ...props }) => (
                                ...
                                code: ({ node, inline, ...props }) => (
                                    inline
                                        ? <code className="bg-theme-bg-secondary px-1 rounded text-xs" {...props} />
                                        : <code className="block bg-theme-bg-secondary p-2 rounded text-xs overflow-x-auto" {...props} />
                                ),
                                p: ({ node, ...props }) => (
